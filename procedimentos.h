...
// Funcao que utiliza polimorfismo para vincular qualquer tipo de Pessoa (Atleta ou Treinador) a um Pais
...
// Realiza o cadastro de um novo pais no sistema
void CadastrarPais(vector<Pais *> &paises) {
    cout << "Insira o nome do país: ";
    string nome = getFormattedStr<string>();
    // Verifica na lista se ja existe um pais com este nome para evitar duplicidade
    ...
    // Verifica se ja existe um pais com esse codigo
    for (auto pais : paises)
        if (pais->getCodigo() == codigo) {
            cout << "Esse código já existe" << endl;
            return;
        }
    // Instancia o novo objeto Pais e o armazena no vetor principal
    Pais *novoPais = new Pais(codigo, nome);
    paises.push_back(novoPais);
}

// Realiza o cadastro de um atleta no sistema
void cadastrarAtleta(vector<Atleta *> &atletas, vector<Pais *> &paises, vector<Modalidade *> &modalidades) {
    ...
    // Tenta vincular o pais. Se falhar, libera a memoria alocada para evitar vazamentos e cancela o cadastro
    if (!associarPessoaPais(novoAtleta, paises)) {
        cout << "O cadastro foi cancelado pois o país é obrigatório." << endl;
        delete novoAtleta;
        return;
    }
    // Permite associar varias modalidades ao atleta
    do {
    ...
    // Armazena o novo atleta no vetor
    atletas.push_back(novoAtleta);
    cout << "\nAtleta cadastrado com sucesso!" << endl;
}

// Realiza o cadastro de um treinador no sistema
void cadastrarTreinador(vector<Treinador *> &treinadores, vector<Pais *> &paises, vector<Modalidade *> &modalidades, vector<Atleta *> &atletas) {
    ...
    // Permite associar varias modalidades ao treinador
    do {
        cout << "Informe o código da modalidade do treinador: " << endl;
        codigoModalidade = getFormattedStr<string>();
        bool modEncontrada = false;
        for (Modalidade *m : modalidades) {
            // Se encontrar a modalidade
            if (m->codigo == codigoModalidade)
            {
     ...
     // Permite associar varios atletas ao treinador
    do
    {
    ...
    // Armazena o novo treinador no vetor
...
void carregarDados(vector<Atleta *> &atletas, vector<Medalha *> &medalhas,
    ...
    //Carrega a modalidade dos atletas
    getline(arquivo, linha); //pula cabeçalho "atleta,quantidade de modalidades,modalidades ->"
    while (getline(arquivo, linha)) {
        if (linha.empty()) break;
